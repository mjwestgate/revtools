...
make_DTM <- function(
  ...
  make_dtm(x, stop_words, min_freq, max_freq, return_matrix)
}
...
make_dtm <- function(
  ...
  # get sum of observations of each term
  lookup$n <- unlist(lapply(
    split(dtm_df$v, dtm_df$j), sum))
  ...
  text_match <- data.frame(
    ...
    final_n = unlist(lapply(text_split,
      function(a){
        if(nrow(a) > 1){
          rep(a$initial_n[order(a$n, decreasing = TRUE)[1]], nrow(a))
        }else{
          a$initial_n
        }
      }))
    )
  lookup <- merge(lookup, text_match, by = "initial_n")
  ...
  # use to create simple_triplet_matrix (faster to run LDA than class:matrix)
